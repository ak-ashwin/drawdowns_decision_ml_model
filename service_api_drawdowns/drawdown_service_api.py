...
cols_needed = ['amount_invoices_last_3_months',
               'no_credit_bounces_last_3_months',
               'overdue_10_to_30_days',
               'value_of_loans_opened_last_12_months_2',
               'amt_bl_paid_off_6',
               'overdue_90_days',
               'last_account_opened_months_13',
               'overdue_gt_10_days',
               'amount_credit_ever_used',
               'invoice_blocks_total_invoice_amount',
               'limits_under_clearing',
               'number_of_active_accounts_53']

lms_limits_cols = ["amount_invoices_last_3_months",
                   "no_credit_bounces_last_3_months",
                   "overdue_10_to_30_days",
                   "overdue_gt_10_days",
                   "amount_credit_ever_used",
...
cibil_cols_required = ["overdue_90_days",
                       "invoice_blocks_total_invoice_amount",
                       "value_of_loans_opened_last_12_months_2",
                       "amt_bl_paid_off_6",
                       "last_account_opened_months_13",
...
data_df = pd.DataFrame(columns=cols_needed)
...
def get_pred(invoice_id):
    ...
    data = scf_invoice.find({"invoice_id": invoice_id}, no_cursor_timeout=True).sort("_id", pymongo.DESCENDING).limit(1)
    for i, row in enumerate(data):
        business_id = str(row['business_id'])
        print("business_id : " + str(business_id))

    lms_loan = db['lms_limits_master']
    data = lms_loan.find({"business_id": business_id}, no_cursor_timeout=True).sort("_id", pymongo.DESCENDING).limit(1)
...
# data_df.to_csv('op1.csv', index=False)
